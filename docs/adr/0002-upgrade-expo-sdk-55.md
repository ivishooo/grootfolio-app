...
- The installed version of Expo Go is for SDK 55.0.0.
...
Expo Go nuevo, el unico SDK que va a poder correr la app es el **54**. Esto
afecta:
...
Migrar el monorepo completo a **Expo SDK 55**, alineando todas las dependencias
mobile y web compatibles. Especificamente:

### Mobile (`apps/mobile/`)

- `expo: ~51.0.39` -> `~55.0.0`
- `react-native: 0.74.5` -> `0.84.1`
- `react: 18.2.0` -> `19.1.0`
- `@types/react: ~18.2.79` -> `~19.1.0`
- `expo-secure-store: ~13.0.2` -> `^14.0.0`
- `expo-status-bar: ~1.12.1` -> `^2.0.0`
- `expo-font: ~12.0.10` -> `^13.0.0`
- `expo-splash-screen: ~0.27.7` -> `^0.30.0`
- `@react-navigation/native: ^6.1.18` -> `^7.0.0`
- `@react-navigation/native-stack: ^6.11.0` -> `^7.0.0`
- `react-native-safe-area-context: 4.10.5` -> `^5.0.0`
- `react-native-screens: 3.31.1` -> `^4.0.0`
- `react-native-svg: 15.2.0` -> `^15.6.0`
- `victory-native: ^36.9.2` -> `^41.0.0` (vuelve a la rama Skia que requeria
  RN 0.78+ y React 19; ahora si calza con RN 0.84)
- `jest-expo: ~51.0.4` -> `~55.0.0`
- `@expo-google-fonts/inter: ^0.2.3` -> `^0.4.0`

Las pins de paquetes secundarios (expo-font, expo-secure-store, etc.) usan
rangos `^` permisivos a proposito; despues de `pnpm install`, se corre
`npx expo install --check --fix` que detecta las versiones exactas que
recomienda Expo SDK 55 y ajusta el `package.json` automaticamente. Es el
flujo oficial de Expo para upgrades de SDK.
...
- `.nvmrc`: `20.11` -> `20.19.4`. SDK 55 + RN 0.84 + Metro 0.83/0.84 requieren
...
- `package.json` raiz: campo `pnpm.overrides` agregado para forzar una unica
  version de `react`, `react-dom`, `@types/react` y `@types/react-dom` en
  todo el monorepo (workspaces + transitivas). Razon: al primer install
  post-upgrade, pnpm resolvio React a versiones distintas en mobile (19.2.0)
  y web (19.0.0) por el juego de peer-deps con react-dom@19.2.5 latest;
  los tipos terminaron mismatched y el typecheck de web fallo con TS2786.
  Pinear via overrides garantiza un solo set de versiones React, lo cual
  ademas es buena practica en monorepos para evitar bugs sutiles de runtime
  cuando dos copias de React conviven.
...
### Migrar a SDK 53 (un punto antes del actual)

- **Pros:** podriamos tener mas tutoriales / Stack Overflow disponibles para
  esa version (suele tomar tiempo que la documentacion se actualice al SDK
  mas nuevo).
- **Contras:** SDK 53 ya esta en linea de actualizaciones de seguridad
  reducidas; los devs que se bajen Expo Go *hoy* tienen 54, no 53; estariamos
  retrasados desde el dia uno.
- **Por que se descarta:** si vamos a migrar, vamos al actual.
...
- Alineacion con el ecosistema actual al momento del trabajo de tesis: React 19,
  RN 0.81, victory-native v41 (Skia-based, mas performante), React Navigation 7.
...
- Defensa de tesis: muestra un proceso de revisar y ajustar decisiones tecnicas
  cuando aparece evidencia nueva, no rigidez con la decision inicial.
...
- **Algun paquete del ecosistema podria no estar todavia actualizado a SDK 55**
  (aunque para los que usamos esto es poco probable). Plan B: pinnear a la
  version compatible mas reciente y abrir issue upstream si el bloqueo persiste.
...
- `README.md`: el bloque de stack se ajusta a React 19 / SDK 55.
- `CLAUDE.md`: idem.
- `.github/dependabot.yml`: las reglas de ignore se ajustan al nuevo pin.

## Plan de migracion (fuera de alcance del ADR pero referenciado)

1. Crear branch `chore/upgrade-expo-sdk-54` desde `develop` (hecho).
2. Bumpear todas las versiones listadas arriba en los `package.json` (este PR).
3. `pnpm install` para regenerar lockfile.
4. Validar `pnpm typecheck && pnpm lint && pnpm build`.
5. Si hay errores de tipos por React 19: arreglar en este mismo PR (no diferir).
6. Mergear a develop con bypass de Repository admin.
7. Rebasear la branch `feature/design-system` (que tiene WIP de Fase 1) sobre
   develop. Ajustar manualmente los pins de fonts mobile que quedaron viejos
   (expo-font 12 -> 14, expo-splash-screen 0.27 -> 0.30, @expo-google-fonts/
   inter 0.2 -> 0.4).
8. Continuar con Fase 1 a partir de la subtarea 17.

## Revision futura

- Cuando salga Expo SDK 55, evaluar la siguiente migracion. Si la fase activa
  del proyecto lo permite (entre fases de implementacion, no en medio de una),
  hacerlo en cuanto el SDK siguiente alcance estabilidad documentada.
